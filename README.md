...
The computational part amounts to calculating a 2D convolution of the grid with a fixed kernel, so the fast Fourier transform comes in handy. The first version of Leniascope used my own implementation of <a href=https://en.wikipedia.org/wiki/Cooley%E2%80%93Tukey_FFT_algorithm>Cooley-Tukey</a>'s radix-2 algorithm, including optimizations accounting for the input and output data being real rather than complex. You can find that implementation in the `fft.cpp` file, and the old code that uses it in the `customfft` branch. The current `master` branch uses the <a href=https://www.fftw.org/>fftw</a> library, which has many fine-tuned optimizations, is several orders of magnitude faster, and is free. Using this, and compiling with the fast floating point option makes it so the speed bottleneck is no longer the fast fourier transform step! It's now evenly divided between the step that applies a color map to the real data, and the step that copies the bitmap into visual memory.
